          ...
          <div class="decoding">
            ...
            <div class="row" style="padding-bottom: 20px;">
              <select style="min-width: 200px; height:45px;" @update:model-value="selectedEngineChanged($event)" v-model="selectedEngineDisplayName">
                <option v-for="config in readerConfigs" :value="config.displayName" v-bind:key="config.displayName">
                  {{ config.displayName }}
                </option>
              </select>
            </div>
...
import { MRZReader, ReaderConfig, DetectionResult, Setting } from "src/reader/MRZReader";
...
const decode = async () => {
  let selectedReaderConfig = getSelectedReaderConfig();
  if (decoding.value === false && selectedReaderConfig) {
    decoding.value = true;
    hasToStop = false;
    await reinitializeReaderIfNeeded();
    await updateReaderSettings(selectedReaderConfig);
...
const addSettingsFileToZip = async (zip:JSZip) => {
  zip.file("settings.json", JSON.stringify(readerConfigs.value));
}
...
const reinitializeReaderIfNeeded = async () => {
  ...
  if (selectedReaderConfig) {
    ...
    if (!reader) {
      ...
      if (reader.getEngine() != selectedReaderConfig.engine) {
        needInitialization = true;
      }
    }
    if (needInitialization) {
      progressLabel.value = "Initializing...";
      reader = await MRZReader.createInstance(selectedReaderConfig.engine);
      progressLabel.value = "";
    }
  }
}
