            ...
            <q-tab-panel name="general">
              <div class="statistics-in-general">
                <div class="charts" style="padding-top:1em;" v-if="Object.keys(readingRateOption).length > 0">
                  ...
                  <div class="chart-container">
                    <v-chart class="chart" :option="scoreOption" />
                  </div>
                  <div class="chart-container">
...
onMounted(async () => {
  ...
  configs = await loadProjectReaderConfigs(router.currentRoute.value.params.name as string);
  const enginesList = [];
  ...
  useMeta({
    // sets document title
    title: 'MRZ Reading Benchmark - '+ projectName.value + ' - Comparison',
  })
...
const getStatistics = async () => {
  ...
  statisticsOfEngines.sort((a, b) => b.metrics.score - a.metrics.score);
  ...
  if (categories.value.length>0) {
    ...
    console.log(statisticsOfCategories);
    calculateCategoryTableRows("readingRate","Reading Rate",{enable:true,mode:"max"},statisticsOfCategories);
    calculateCategoryTableRows("score","Score",{enable:true,mode:"max"},statisticsOfCategories);
  ...
  const readingRate = getData(statisticsOfEngines,"readingRate");
  const averageTimeData = getData(statisticsOfEngines,"averageTime");
  const scoreData = getData(statisticsOfEngines,"score");
  readingRateOption.value = getOptionForChart(readingRate,"Reading Rate","{c}%",sortedEngineNames);
  averageTimeOption.value = getOptionForChart(averageTimeData,"Average Time","{c}ms",sortedEngineNames);
  scoreOption.value = getOptionForChart(scoreData,"Score","{c}%",sortedEngineNames);
  ...
  if (statisticsOfEngines[0].rows) {
    for (let i = 0; i < statisticsOfEngines[0].rows.length; i++) {
      ...
      for (let j = 0; j < statisticsOfEngines.length; j++) {
        ...
        if (statistics.rows) {
          if (statistics.rows[i].score === 100.0) {
